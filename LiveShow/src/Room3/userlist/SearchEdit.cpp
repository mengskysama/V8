...
// CSearchEdit
#define DEFAULT_TEXT _T("输入用户昵称/白兔KTV号码")
...
BEGIN_MESSAGE_MAP(CSearchEdit, CEdit)
	...
	ON_WM_SETFOCUS()
	ON_WM_KILLFOCUS()
	ON_CONTROL_REFLECT_EX(EN_CHANGE, &CSearchEdit::OnEnChange)
	ON_WM_MOUSEWHEEL()
	ON_WM_LBUTTONDOWN()
	ON_WM_MOUSEMOVE()
	ON_WM_CREATE()
...
LRESULT CSearchEdit::OnMouseHover(WPARAM wParam,LPARAM lParam)
{
	//CPoint   pt;   
	//GetCursorPos(&pt);
	////m_ToolTip.Show(_T("输入好友备注、昵称、51用户名、彩虹号进行查找"), this, &pt);
	//ShowToolTip(_T("输入好友备注、昵称、51用户名、彩虹号进行查找"));
	return 0;
}	
...
void CSearchEdit::OnMouseMove(UINT nFlags, CPoint point)
{
	...
	if (!m_bTipShow)
	{
		...
		GetCursorPos(&pt);
		//m_ToolTip.Show(_T("输入好友备注、昵称、51用户名、彩虹号进行查找"), this, &pt);
		//ShowToolTip(_T("输入小组成员的群名片、昵称、彩虹号进行查找"));
	}
