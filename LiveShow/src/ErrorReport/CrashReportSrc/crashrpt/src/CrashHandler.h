...
#include "../../../include/crashrpt.h"      // defines LPGETLOGFILE callback
...
typedef struct tagREPORTOPTION
{
	CString strSmtp;
	CString strDlgTitle;
	CString strUsername;
	CString strPassword;
	CString strBody;
	CString strExternalRpt;
	CString strAlias;
	bool bSendAll;
	bool bSilent;
	ErrorReportCallback func;
	tagREPORTOPTION()
	{
		func=NULL;
	}
...
public:
   ...
   CCrashHandler(
	  ...
	  LPCTSTR lpcszAppName = NULL,			// 应用程序名称
	  LPCTSTR lpVersion = NULL				// 版本号
      );
...
public:
	void CallUserCallback(int status);
};
