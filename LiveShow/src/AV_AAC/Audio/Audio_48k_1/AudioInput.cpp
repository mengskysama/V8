...
/*
	...
	*/
	...
	if (umtType != MessageHandler::UDPVoiceSpeex) {
		...
		if (! m_de_cCodec) {
			m_de_cCodec = new CELTCodec;
			if (m_de_cCodec) {
				m_de_cdDecoder = m_de_cCodec->decoderCreate();
			}
		}
		
		static celt_int16 fout2[4096]={0};

		if (m_de_cCodec)
		{
			int len3 =m_de_cCodec->decode(m_de_cdDecoder, buffer, len, fout2, SAMPLE_RATE / 100);
			len3++;

			UINT dwDataWrote;
			if( FAILED(g_pWaveFile.Write( SAMPLE_RATE / 100*2, (BYTE*)fout2, 
				&dwDataWrote ) ))
			{
				int a=0;
				a++;
