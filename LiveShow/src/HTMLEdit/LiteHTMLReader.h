 ...
 *       there is only one.
 */
class HTMLEDIT_API CLiteHTMLReader
{
public:
	enum EventMaskEnum {
		/** @since 1.0 */ 
...
inline bool CLiteHTMLReader::parseComment(CString &rComment)
{
	...
	//ASSERT(m_dwBufPos + 4 < m_dwBufLen);   by ken 11 03 2006 代码走到这里时，不一定满足这个assert条件，但不满足不是错误
									  ...
									  bool &bIsClosingTag)
{
	ASSERT(m_lpszBuffer != NULL);
	ASSERT(m_dwBufPos >= 0L);
	//ASSERT(m_dwBufPos + 4 < m_dwBufLen);  by ken 11 02 2006     
