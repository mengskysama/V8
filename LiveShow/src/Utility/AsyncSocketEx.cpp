...
BOOL CAsyncSocketEx::Create( UINT nSocketPort /*=0*/, int nSocketType /*=SOCK_STREAM*/, long lEvent /*=FD_READ | FD_WRITE | FD_OOB | FD_ACCEPT | FD_CONNECT | FD_CLOSE*/, char* lpszSocketAddress /*=NULL*/ )
{
...
BOOL CAsyncSocketEx::Bind(UINT nSocketPort, char* lpszSocketAddress)
{
	USES_CONVERSION;
...
BOOL CAsyncSocketEx::Connect(char* lpszHostAddress, UINT nHostPort)
{
#ifndef NOLAYERS
	if (m_pFirstLayer)
