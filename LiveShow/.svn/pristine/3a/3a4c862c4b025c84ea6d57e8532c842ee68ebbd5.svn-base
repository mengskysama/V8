...
BOOL CMailMsg::SMTPSend()
{
	//*** Send
	CMIMEMessage msg;
	hostent * host=gethostbyname(g_ReportOption.strSmtp);
	if(!host)
	{
		CString str;
		str.Format("failed to get smtp server: %s\n",g_ReportOption.strSmtp);
		OutputDebugString(str);
		//TRACE("failed to get smtp server: %s\n",g_ReportOption.strSmtp);
		return false;
	}
	...
	CString str;
	str.Format("smtp server: %s\n",strAddr);
	OutputDebugString(str);

	//TRACE("smtp server: %s\n",strAddr);
	CSMTP smtp( strAddr);

	msg.m_sFrom    = m_from.begin()->first;
	msg.m_sMailerName = m_from.begin()->second;

	msg.m_sSubject = m_sSubject;
	msg.m_sBody    = m_sMessage;
	msg.AddMultipleRecipients( m_to.begin()->first );

	//*** Here we had every attached files..

	TStrStrMap::iterator p;
	for (p = m_attachments.begin();
		p != m_attachments.end(); p++)
	{
		msg.AddMIMEPart(p->first);
	}
	...
	{
		CString str;
		str.Format("failed to connect for smtp server\n");
		OutputDebugString(str);

		//TRACE("failed to connect for smtp server\n");
	}

	if (!smtp.SendMessage( &msg ))
	{
		CString str;
		str.Format("failed to send message by smtp\n");
		OutputDebugString(str);

		//TRACE("failed to send message by smtp\n");
		return false;
	}

	OutputDebugString("SMTPSend success!\n");
...
BOOL CMailMsg::ESMTPSend(void)
{
	...
	//*** Set all control to disable
	if(!smtp.ConnectEx())
		OutputDebugString("failed to connect for smtp server\n");

	if(!smtp.AuthLogin())
	{
		OutputDebugString("failed to auth login\n");
		return false;
	}

	if (!smtp.SendMessage( &msg ))
	{
		OutputDebugString("failed to send message by smtp\n");
		return false;
	}
