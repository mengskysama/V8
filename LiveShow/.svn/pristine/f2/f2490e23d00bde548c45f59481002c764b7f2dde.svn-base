...
char * URL::pre_encode(const char *pre_url)
{
	char *str_new;
	int str_new_len;
	char *ptr, *pptr;
...
char* URL::encode(const char* url)
{
	char *url_enc;
	int url_enc_len;
	char *ptr, *pptr;
...
int URL::_parse_fragment(char* &url)
{
	char *ptr;
...
int URL::_parse_scheme(char* &url)
{
	//char *ptr;
...
int URL::_parse_location(char* &url)
{
...
int URL::_parse_query_info(char* &url)
{
	char *ptr;
...
int URL::_parse_parameters(char* &url)
{
	char *ptr;
...
int URL::_parse(const char *url_orig)
{
	char *url_parsed;
	char buf[1024];
...
int URL::set_url(const char *url_orig)
{
	free_all();
	return _parse(url_orig);
};

void URL::free_all(void)
{
	delete[] url; url = NULL;
...
int URL::reset_url(const char *url_orig)
{
	assert(url_orig);
