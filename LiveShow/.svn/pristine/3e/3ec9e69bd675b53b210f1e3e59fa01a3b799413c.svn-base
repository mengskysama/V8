...
/////////////////////////////////////////////////////////////////////////////
// CRectTrackerEx intitialization
...
void CRectTrackerEx::SetCtrl(CTrackerCtrl *pCtrl)
{
	m_pCtrl = pCtrl;
}

void CRectTrackerEx::Construct()
{
	...
	if (!bInitialized)
	{
		...
		if (_afxBlackDottedPen == NULL)
		{
			...
			_afxBlackDottedPen = CreatePen(PS_SOLID, 0, RGB(255,255,255));
			if (_afxBlackDottedPen == NULL)
			{
				AfxUnlockGlobals(CRIT_RECTTRACKER);
				AfxThrowResourceException();
...
void CRectTrackerEx::Draw(CDC* pDC) const
{
	...
	if ((m_nStyle & (dottedLine|solidLine)) != 0)
	{
		...
		_afxBlackDottedPen = CreatePen(PS_SOLID, 0, RGB(0,0,0));
		if (_afxBlackDottedPen == NULL)
		{
			AfxUnlockGlobals(CRIT_RECTTRACKER);
			AfxThrowResourceException();
...
int CRectTrackerEx::HitTest(CPoint point) const
{
...
int CRectTrackerEx::NormalizeHit(int nHandle) const
{
...
//void CRectTrackerEx::AdjustRect(int nHandle, LPRECT)
//{
...
void CRectTrackerEx::GetTrueRect(LPRECT lpTrueRect) const
{
...
/////////////////////////////////////////////////////////////////////////////
// CRectTrackerEx implementation helpers

void CRectTrackerEx::GetHandleRect(int nHandle, CRect* pHandleRect) const
{
	ASSERT(nHandle < 8);
...
int CRectTrackerEx::GetHandleSize(LPCRECT lpRect) const
{
	if (lpRect == NULL)
		lpRect = &m_rect;
		...
		case WM_MOUSEMOVE:
			...
			if (!rectOld.EqualRect(&m_rect) || m_bFinalErase)
			{
				...
				if (msg.message != WM_LBUTTONUP)
					bMoved = TRUE;
				m_pCtrl->SendMessage(WM_TRACKER_MOVE,0,0);
			}
			if (m_bFinalErase)
			{
				m_pCtrl->SetDrag(false);
				goto ExitLoop;
			}
...
void CRectTrackerEx::GetModifyPointers(
...
void CRectTrackerEx::SetLimitRect(CRect limitRect)
{
...
{
	int del;
	del = lpRect->left - m_LimitRect.left;
	if (del<0)
	{
		lpRect->left = m_LimitRect.left;
		lpRect->right -= del;
	}
	del = lpRect->right - m_LimitRect.right;
	if (del>0)
	{
	...
	del = lpRect->top - m_LimitRect.top;
	if (del<0)
	{
	...
	del = lpRect->bottom - m_LimitRect.bottom;
	if (del>0)
	{
...
//CRect CRectTrackerEx::GetRealRect()
//{
//	return m_RealRect;
//}
