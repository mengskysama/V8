...
void CAsyncSocketExLayer::OnClose(int nErrorCode)
{
	if (m_pPrevLayer)
		...
		if (m_pOwnerSocket->m_lEvent&FD_CLOSE)
		{
			...
			{
				PostMessage(m_hNotifyWnd, WM_PROXY_TEST, (WPARAM)false, 0);
			}
		}
}
...
BOOL CAsyncSocketExLayer::Connect(char* lpszHostAddress, UINT nHostPort)
{
...
BOOL CAsyncSocketExLayer::ConnectNext(char* lpszHostAddress, UINT nHostPort)
{
	...
	else
	{
		...
		char* lpszAscii = lpszHostAddress;
...
BOOL CAsyncSocketExLayer::Create(UINT nSocketPort, int nSocketType,
			long lEvent, char* lpszSocketAddress)
{
